...
export class DefaultCmabService implements CmabService {
  ...
  async getVariation(
    ...
    const filteredAttributes = this.filterAttributes(projectConfig, userContext, experimentId);

    if (options.includes(OptimizelyDecideOption.IGNORE_CMAB_CACHE)) {
      return this.fetchVariation(experimentId, userContext.getUserId(), filteredAttributes);
    }
    ...
    const cacheKey = this.getCacheKey(userContext.getUserId(), experimentId);
    ...
    const variation = await this.fetchVariation(experimentId, userContext.getUserId(), filteredAttributes);
  ...
  private async fetchVariation(
    ...
    const variationId = await this.cmabClient.fetchVariation(experimentId, userId, attributes, cmabUuid);
  ...
  private getCacheKey(userId: string, experimentId: string): string {
    ...
    return `${len}-${userId}-${experimentId}`;
  }
}
