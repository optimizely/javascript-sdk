...
import * as projectConfigManager from '../project_config/project_config_manager';
...
const getMockEventProcessor = (notificationCenter) => {
  return getForwardingEventProcessor(getMockEventDispatcher(), notificationCenter);
}

const getMockErrorNotifier = () => {
  return {
    notify: sinon.spy(),
  }
};
...
const getOptlyInstance = ({ datafileObj, defaultDecideOptions }) => {
  ...
  const errorNotifier = getMockErrorNotifier();

  const notificationCenter = createNotificationCenter({ logger: createdLogger, errorNotifier });
  ...
  return { optlyInstance, eventProcessor, eventDispatcher, notificationCenter, errorNotifier, createdLogger }
}
...
describe('lib/optimizely', function() {
  ...
  describe('constructor', function() {
    ...
    describe('constructor', function() {
      it('should log if the client engine passed in is invalid', function() {
        ...
        sinon.assert.called(createdLogger.info);

        assert.deepEqual(createdLogger.info.args[0], [INVALID_CLIENT_ENGINE, undefined]);
      });
      ...
      it('should log if the defaultDecideOptions passed in are invalid', function() {
        ...
        sinon.assert.called(createdLogger.debug);
        assert.deepEqual(createdLogger.debug.args[0], [INVALID_DEFAULT_DECIDE_OPTIONS]);
      });
      ...
      describe('when a user profile service is provided', function() {
        ...
        it('should pass in a null user profile to the decision service if the provided user profile is invalid', function() {
          ...
          // );
          sinon.assert.called(createdLogger.warn);
        });
      });
    });
  });
  ...
  describe('APIs', function() {
    ...
    var eventProcessor = getForwardingEventProcessor(eventDispatcher, notificationCenter);
    ...
    describe('#activate', function() {
      it('should call bucketer and dispatchEvent with proper args and return variation key', function() {
        ...
        var expectedObj = {
          ...
          params: {
            ...
            client_version: enums.CLIENT_VERSION,
            anonymize_ip: false,
      ...
      it('should throw an error for invalid user ID', function() {
        ...
        // assert.strictEqual(logMessage1, sprintf(INVALID_INPUT_FORMAT, 'OPTIMIZELY', 'user_id'));
      ...
      it('should throw an error for invalid attributes', function() {
        ...
        // sinon.assert.calledTwice(createdLogger.log);
        // var logMessage1 = buildLogMessageFromArgs(createdLogger.log.args[0]);
        // assert.strictEqual(logMessage1, sprintf(INVALID_ATTRIBUTES, 'ATTRIBUTES_VALIDATOR'));
        // var logMessage2 = buildLogMessageFromArgs(createdLogger.log.args[1]);
        // assert.strictEqual(
        //   logMessage2,
        //   sprintf(NOT_ACTIVATING_USER, 'OPTIMIZELY', 'testUser', 'testExperimentWithAudiences')
        // );
        sinon.assert.calledWithExactly(
          createdLogger.info,
          NOT_ACTIVATING_USER,
    ...
    describe('#track', function() {
      ...
      it('should not call dispatchEvent when including invalid event value', function() {
        ...
        sinon.assert.notCalled(eventDispatcher.dispatchEvent);
        sinon.assert.calledOnce(createdLogger.error);
      });
      ...
      it('should throw an error for invalid attributes', function() {
        ...
        // sinon.assert.calledOnce(createdLogger.log);
        // var logMessage = buildLogMessageFromArgs(createdLogger.log.args[0]);
        // assert.strictEqual(logMessage, sprintf(INVALID_ATTRIBUTES, 'ATTRIBUTES_VALIDATOR'));
      });
      ...
      it('should not track when optimizely object is not a valid instance', function() {
        ...
        sinon.assert.calledOnce(createdLogger.log);
        var logMessage = buildLogMessageFromArgs(createdLogger.log.args[0]);
        assert.strictEqual(logMessage, sprintf(INVALID_OBJECT, 'OPTIMIZELY', 'track'));
    ...
    describe('#getVariation', function() {
      ...
      it('should log an error for invalid experiment key', function() {
        ...
        assert.strictEqual(
          logMessage,
          sprintf(INVALID_EXPERIMENT_KEY, 'OPTIMIZELY', 'invalidExperimentKey')
        );
      });
      ...
      describe('whitelisting', function() {
        ...
        it('should return forced variation after experiment status check and before audience check', function() {
          ...
          assert.strictEqual(
            logMessage0,
            sprintf(USER_HAS_NO_FORCED_VARIATION, 'DECISION_SERVICE', 'user1')
          );
          var logMessage = buildLogMessageFromArgs(createdLogger.log.args[1]);
          assert.strictEqual(
            logMessage,
            sprintf(USER_FORCED_IN_VARIATION, 'DECISION_SERVICE', 'user1', 'control')
          );
        });
      });
      ...
      it('should not return variation when optimizely object is not a valid instance', function() {
        ...
        sinon.assert.calledOnce(createdLogger.log);
        var logMessage = buildLogMessageFromArgs(createdLogger.log.args[0]);
        assert.strictEqual(logMessage, sprintf(INVALID_OBJECT, 'OPTIMIZELY', 'getVariation'));
    ...
    describe('#getForcedVariation', function() {
      it('should return null when set has not been called', function() {
        ...
        var logMessage = buildLogMessageFromArgs(createdLogger.log.args[0]);
        assert.strictEqual(logMessage, sprintf(USER_HAS_NO_FORCED_VARIATION, 'DECISION_SERVICE', 'user1'));
      });
      ...
      it('should return null with a null experimentKey', function() {
        ...
        var logMessage = buildLogMessageFromArgs(createdLogger.log.args[0]);
        assert.strictEqual(logMessage, sprintf(INVALID_INPUT_FORMAT, 'OPTIMIZELY', 'experiment_key'));
      });
    ...
    describe('#setForcedVariation', function() {
      it('should be able to set a forced variation', function() {
        ...
        assert.strictEqual(
          logMessage,
          sprintf(USER_MAPPED_TO_FORCED_VARIATION, 'DECISION_SERVICE', 111128, 111127, 'user1')
        );
      });
      ...
      it('should be able to set, unset, and get forced variation', function() {
        ...
        assert.strictEqual(
          ...
          sprintf(USER_HAS_FORCED_VARIATION, 'DECISION_SERVICE', 'control', 'testExperiment', 'user1')
        );

        assert.strictEqual(
          variationMappingRemovedLogMessage,
          sprintf(VARIATION_REMOVED_FOR_USER, 'DECISION_SERVICE', 'testExperiment', 'user1')
        );
      });
      ...
      it('should not set an invalid experiment', function() {
        ...
        assert.strictEqual(
          logMessage,
          sprintf(EXPERIMENT_KEY_NOT_IN_DATAFILE, 'PROJECT_CONFIG', 'definitely_not_valid_exp_key')
        );
      });
      ...
      it('should return false for a null experimentKey', function() {
        ...
        assert.strictEqual(
          setVariationLogMessage,
          sprintf(INVALID_INPUT_FORMAT, 'OPTIMIZELY', 'experiment_key')
        );
      });
      ...
      it('should return false for a null userId', function() {
        ...
        assert.strictEqual(
          setVariationLogMessage,
          sprintf(INVALID_INPUT_FORMAT, 'OPTIMIZELY', 'user_id')
        );
      });
      ...
      it('should return false for a null variationKey', function() {
        ...
        assert.strictEqual(
          setVariationLogMessage,
          sprintf(USER_NOT_IN_FORCED_VARIATION, 'DECISION_SERVICE', 'user1')
        );
      });
      ...
      it('should not override check for not running experiments in getVariation', function() {
        ...
        assert.strictEqual(
          logMessage0,
          sprintf(USER_MAPPED_TO_FORCED_VARIATION, 'DECISION_SERVICE', 133338, 133337, 'user1')
        );

        var logMessage1 = buildLogMessageFromArgs(createdLogger.log.args[1]);
        assert.strictEqual(
          logMessage1,
          sprintf(EXPERIMENT_NOT_RUNNING, 'DECISION_SERVICE', 'testExperimentNotRunning')
        );
      });
    });
  ...
  describe('decide APIs', function() {
    ...
    describe('#decide', function() {
      ...
      describe('with empty default decide options', function() {
        let optlyInstance, notificationCenter, createdLogger;
        beforeEach(function() {
        ...
        it('should return error decision object when SDK is not ready and do not dispatch an event', function() {
          optlyInstance.projectConfigManager.getConfig = sinon.stub().returns(null);
        ...
        it('should make a decision for feature_test and dispatch an event', function() {
          ...
          sinon.assert.callCount(notificationCenter.sendNotifications, 4);
          var notificationCallArgs = notificationCenter.sendNotifications.getCall(3).args;
        ...
        it('should make a decision and do not dispatch an event with DISABLE_DECISION_EVENT passed in decide options', function() {
          ...
          assert.deepEqual(decision, expectedDecision);
          sinon.assert.notCalled(eventDispatcher.dispatchEvent);
          sinon.assert.calledTwice(notificationCenter.sendNotifications);
          var notificationCallArgs = notificationCenter.sendNotifications.getCall(1).args;
          var expectedNotificationCallArgs = [
            NOTIFICATION_TYPES.DECISION,
            {
              type: 'flag',
        ...
        it('should make a decision with excluded variables and do not dispatch an event with DISABLE_DECISION_EVENT and EXCLUDE_VARIABLES passed in decide options', function() {
          ...
          assert.deepEqual(decision, expectedDecision);
          sinon.assert.notCalled(eventDispatcher.dispatchEvent);
          sinon.assert.calledOnce(notificationCenter.sendNotifications);
          var notificationCallArgs = notificationCenter.sendNotifications.getCall(0).args;
          var expectedNotificationCallArgs = [
            NOTIFICATION_TYPES.DECISION,
            {
              type: 'flag',
        ...
        it('should make a decision for rollout and do not dispatch an event when sendFlagDecisions is set to false', function() {
          ...
          optlyInstance.projectConfigManager.getConfig = sinon.stub().returns(newConfig);
          var flagKey = 'feature_1';
          var expectedVariables = optlyInstance.getAllFeatureVariables(flagKey, userId);
          var user = new OptimizelyUserContext({
            optimizely: optlyInstance,
      ...
      describe('with EXCLUDE_VARIABLES flag in default decide options', function() {
        it('should exclude variables in decision object and dispatch an event', function() {
          ...
          sinon.assert.calledOnce(eventDispatcher.dispatchEvent);
          sinon.assert.calledThrice(notificationCenter.sendNotifications);
      ...
      describe('with INCLUDE_REASONS flag in default decide options', function() {
        ...
        it('should include reason when experiment is not running', function() {
          ...
          expect(decision.reasons).to.include(
            sprintf(EXPERIMENT_NOT_RUNNING, 'DECISION_SERVICE', 'exp_with_audience')
          );
        });
        ...
        it('should include reason when returning stored variation from user profile', function() {
          ...
          expect(decision.reasons).to.include(
            sprintf(RETURNING_STORED_VARIATION, 'DECISION_SERVICE', variationKey2, experimentKey, userId)
          );
        });
        ...
        it('should include reason when user is forced in variation', function() {
          ...
          var decision = optlyInstance.decide(user, flagKey);
          expect(decision.reasons).to.include(
            sprintf(USER_FORCED_IN_VARIATION, 'DECISION_SERVICE', userId, variationKey)
          );
        });
        ...
        it('should include reason when user has forced variation', function() {
          ...
          var decision = optlyInstance.decide(user, flagKey);
          expect(decision.reasons).to.include(
            sprintf(USER_HAS_FORCED_VARIATION, 'DECISION_SERVICE', variationKey, experimentKey, userId)
          );
        });
        ...
        it('should include reason when invalid forced variation found', function() {
          ...
          var decision = optlyInstance.decide(user, flagKey);
          expect(decision.reasons).to.include(
            sprintf(FORCED_BUCKETING_FAILED, 'DECISION_SERVICE', variationKey, userId)
          );
        });
        ...
        it('should include reason when user meets conditions for targeting rule', function() {
          ...
          var decision = optlyInstance.decide(user, flagKey);
          expect(decision.reasons).to.include(
            sprintf(USER_MEETS_CONDITIONS_FOR_TARGETING_RULE, 'DECISION_SERVICE', userId, '1')
          );
        });
        ...
        it('should include reason when user does not meet conditions for targeting rule', function() {
          ...
          var decision = optlyInstance.decide(user, flagKey);
          expect(decision.reasons).to.include(
            sprintf(USER_DOESNT_MEET_CONDITIONS_FOR_TARGETING_RULE, 'DECISION_SERVICE', userId, '1')
          );
        });
        ...
        it('should include reason when user is bucketed into targeting rule', function() {
          ...
          var decision = optlyInstance.decide(user, flagKey);
          expect(decision.reasons).to.include(
            sprintf(USER_IN_ROLLOUT, 'DECISION_SERVICE', userId, flagKey)
          );
        });
        ...
        it('should include reason when user is bucketed into everyone targeting rule', function() {
          ...
          var decision = optlyInstance.decide(user, flagKey);
          expect(decision.reasons).to.include(
            sprintf(USER_MEETS_CONDITIONS_FOR_TARGETING_RULE, 'DECISION_SERVICE', userId, 'Everyone Else')
          );
        });
        ...
        it('should include reason when user is not bucketed into targeting rule', function() {
          ...
          var decision = optlyInstance.decide(user, flagKey);
          expect(decision.reasons).to.include(
            sprintf(USER_NOT_BUCKETED_INTO_TARGETING_RULE, 'DECISION_SERVICE', userId, '2')
          );
        });
        ...
        it('should include reason when user is bucketed into variation of experiment', function() {
          ...
          var decision = optlyInstance.decide(user, flagKey);
          expect(decision.reasons).to.include(
            sprintf(USER_HAS_VARIATION, 'DECISION_SERVICE', userId, variationKey, experimentKey)
          );
        });
        ...
        it('should include reason when user is bucketed into experiment in group', function() {
          ...
          var decision = optlyInstance.decide(user, flagKey);
          expect(decision.reasons).to.include(
            sprintf(USER_BUCKETED_INTO_EXPERIMENT_IN_GROUP, 'BUCKETER', userId, experimentKey, groupId)
          );
        });
        ...
        it('should include reason when user is not attached to any experiment', function() {
          ...
          var decision = optlyInstance.decide(user, flagKey);
          expect(decision.reasons).to.include(
            sprintf(FEATURE_HAS_NO_EXPERIMENTS, 'DECISION_SERVICE', flagKey)
          );
        });
        ...
        it('should include reason when user is not in experiment', function() {
          ...
          var decision = optlyInstance.decide(user, flagKey);
          expect(decision.reasons).to.include(
            sprintf(USER_NOT_IN_EXPERIMENT, 'DECISION_SERVICE', userId, experimentKey)
          );
        });
    ...
    describe('#decideForKeys', function() {
      ...
      it('should return decision results map with single flag key provided for feature_test and dispatch an event', function() {
        var flagKey = 'feature_2';
        const { optlyInstance, eventDispatcher } = getOptlyInstance({ datafileObj: testData.getTestDecideProjectConfig() });
        var user = optlyInstance.createUserContext(userId);
        var expectedVariables = optlyInstance.getAllFeatureVariables(flagKey, userId);
  ...
  describe('feature management', function() {
    ...
    describe('feature variable APIs', function() {
      ...
      it('returns null from getFeatureVariable when optimizely object is not a valid instance', function() {
        ...
        sinon.assert.calledOnce(createdLogger.log);
        var logMessage = buildLogMessageFromArgs(createdLogger.log.args[0]);
        assert.strictEqual(logMessage, sprintf(INVALID_OBJECT, 'OPTIMIZELY', 'getFeatureVariable'));
      });
      ...
      it('returns null from getFeatureVariableBoolean when optimizely object is not a valid instance', function() {
        ...
        sinon.assert.calledOnce(createdLogger.log);
        var logMessage = buildLogMessageFromArgs(createdLogger.log.args[0]);
        assert.strictEqual(logMessage, sprintf(INVALID_OBJECT, 'OPTIMIZELY', 'getFeatureVariableBoolean'));
      });
      ...
      it('returns null from getFeatureVariableDouble when optimizely object is not a valid instance', function() {
        ...
        sinon.assert.calledOnce(createdLogger.log);
        var logMessage = buildLogMessageFromArgs(createdLogger.log.args[0]);
        assert.strictEqual(logMessage, sprintf(INVALID_OBJECT, 'OPTIMIZELY', 'getFeatureVariableDouble'));
      });
      ...
      it('returns null from getFeatureVariableInteger when optimizely object is not a valid instance', function() {
        ...
        sinon.assert.calledOnce(createdLogger.log);
        var logMessage = buildLogMessageFromArgs(createdLogger.log.args[0]);
        assert.strictEqual(logMessage, sprintf(INVALID_OBJECT, 'OPTIMIZELY', 'getFeatureVariableInteger'));
      });
      ...
      it('returns null from getFeatureVariableString when optimizely object is not a valid instance', function() {
        ...
        sinon.assert.calledOnce(createdLogger.log);
        var logMessage = buildLogMessageFromArgs(createdLogger.log.args[0]);
        assert.strictEqual(logMessage, sprintf(INVALID_OBJECT, 'OPTIMIZELY', 'getFeatureVariableString'));
      });
      ...
      it('returns null from getFeatureVariableJSON when optimizely object is not a valid instance', function() {
        ...
        sinon.assert.calledOnce(createdLogger.log);
        var logMessage = buildLogMessageFromArgs(createdLogger.log.args[0]);
        assert.strictEqual(logMessage, sprintf(INVALID_OBJECT, 'OPTIMIZELY', 'getFeatureVariableJSON'));
      });
    });
  });
  ...
  describe('audience match types', function() {
    ...
    beforeEach(function() {
      const mockConfigManager = getMockProjectConfigManager({
        initConfig: createProjectConfig(testData.getTypedAudiencesConfig()),
      });
  ...
  describe('event batching', function() {
    ...
    beforeEach(function() {
      ...
      eventProcessor = getForwardingEventProcessor(
        eventDispatcher,
      );
    });
    ...
    describe('close method', function() {
      ...
      beforeEach(function() {
        mockEventProcessor = {
          ...
          start: sinon.stub(),
          stop: sinon.stub(),
          onRunning: sinon.stub(),
          onTerminated: sinon.stub(),
          onDispatch: sinon.stub(),
        };
      });

      describe('when the event processor onTerminated method returns a promise that fulfills', function() {
        beforeEach(function() {
          eventProcessorStopPromise = Promise.resolve();
          mockEventProcessor.onTerminated.returns(eventProcessorStopPromise);
          const mockConfigManager = getMockProjectConfigManager({
            initConfig: createProjectConfig(testData.getTestProjectConfig()),
          });
      ...
      describe('when the event processor onTerminated() method returns a promise that rejects', function() {
        beforeEach(function() {
          eventProcessorStopPromise = Promise.reject(new Error(FAILED_TO_STOP));
          eventProcessorStopPromise.catch(() => {});
          mockEventProcessor.onTerminated.returns(eventProcessorStopPromise);
          const mockConfigManager = getMockProjectConfigManager({
            initConfig: createProjectConfig(testData.getTestProjectConfig()),
          });
  ...
  describe('project config management', function() {
    ...
    it('should call the project config manager stop method when the close method is called', function() {
      ...
      sinon.stub(projectConfigManager, 'stop');
    ...
    describe('onReady method', function() {
      ...
      it('rejects the promise after the timeout has expired when the project config manager onReady promise still has not resolved', function() {
        ...
        return readyPromise.then(() => {
          ...
          assert.equal(err.message, sprintf(ONREADY_TIMEOUT_EXPIRED, 500));
        });
      });
  ...
  describe('log event notification', function() {
    ...
    var notificationCenter = createNotificationCenter({ logger });
    var eventProcessor;
    beforeEach(function() {
      bucketStub = sinon.stub(bucketer, 'bucket');
      eventDispatcherSpy = sinon.spy(() => Promise.resolve({ statusCode: 200 }));
      eventProcessor = getForwardingEventProcessor(
