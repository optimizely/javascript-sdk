...
describe('lib/core/project_config', function() {
  describe('createProjectConfig method', function() {
    ...
    describe('flag variations', function() {
      ...
      it('it should populate flagVariationsMap correctly', function() {
        ...
        assert.deepEqual(feature1VariationsKeys.sort(), ['18257766532', '3324490562', '3324490633', 'a', 'b']);
        assert.deepEqual(feature2VariationsKeys.sort(), ['variation_no_traffic', 'variation_with_traffic']);
  ...
  describe('projectConfig helper methods', function() {
    ...
    describe('#isFeatureExperiment', function() {
      it('returns true for a feature test', function() {
        var config = projectConfig.createProjectConfig(JSON.stringify(testDatafile.getTestProjectConfigWithFeatures()));
      ...
      it('returns false for an A/B test', function() {
        var config = projectConfig.createProjectConfig(JSON.stringify(testDatafile.getTestProjectConfig()));
      ...
      it('returns true for a feature test in a mutex group', function() {
        var config = projectConfig.createProjectConfig(JSON.stringify(testDatafile.getMutexFeatureTestsConfig()));
  ...
  describe('integrations', () => {
    describe('#withSegments', () => {
      var config;
      beforeEach(() => {
        config = projectConfig.createProjectConfig(JSON.stringify(testDatafile.getOdpIntegratedConfigWithSegments()));
      });
    ...
    describe('#withoutValidIntegrationKey', () => {
      it('should throw an error when parsing the project config due to integrations not containing a key', () => {
        ...
        assert.throws(() => {
          projectConfig.createProjectConfig(JSON.stringify(odpIntegratedConfigWithoutKey));
        });
      });
    });
...
describe('#createProjectConfig with options', function() {
  ...
  it('returns a project config object when all validation is applied and there are no errors', function() {
    var configDatafile = {
      version: '4',
      foo: 'bar',
    ...
    var result = projectConfig.createProjectConfig(JSON.stringify(configDatafile), {
  ...
  it('throws an error when datafile is invalid JSON', function() {
    stubJsonSchemaValidator.returns(true);
    assert.throws(() => {
  ...
  it('throws an error when jsonSchemaValidator.validate throws', function() {
    var configDatafile = { version: '4', foo: 'bar' };
