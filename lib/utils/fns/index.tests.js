...
import { isNumber, isSafeInteger, keyBy } from './';
  ...
  describe('APIs', function() {
    describe('isFinite', function() {
      it('should return false for invalid numbers', function() {
        ...
        assert.isFalse(isSafeInteger('3'));
        assert.isFalse(isSafeInteger(Math.pow(2, 53) + 2));
        assert.isFalse(isSafeInteger(-Math.pow(2, 53) - 2));
      });

      it('should return true for valid numbers', function() {
        assert.isTrue(isSafeInteger(0));
        assert.isTrue(isSafeInteger(10));
        assert.isTrue(isSafeInteger(10.5));
        assert.isTrue(isSafeInteger(Math.pow(2, 53)));
        assert.isTrue(isSafeInteger(-Math.pow(2, 53)));
      });
    });
    ...
    describe('keyBy', function() {
      ...
      it('should return empty object when first argument is null or undefined', function() {
        var obj = keyBy(null, 'key1');
        assert.isEmpty(obj);

        obj = keyBy(undefined, 'key1');
        assert.isEmpty(obj);
      });
    });
    ...
    describe('isNumber', function() {
      ...
      it('should return true in case of value from Number object ', function() {
        assert.isTrue(isNumber(Number.MIN_VALUE));
      });
