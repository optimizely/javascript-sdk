...
### Fixed
- `stop` method of `EventProcessor` now reflects the status of all prior in-flight dispatcher requests, not just the final request that was triggered at the time `stop` was called.
