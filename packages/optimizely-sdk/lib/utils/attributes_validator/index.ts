...
import { ERROR_MESSAGES, VERSION_TYPE, LOG_MESSAGES } from '../enums';
 ...
 *                     
 */
function isBuildVersion(version: unknown): boolean {
  if (typeof version != 'string') {
    return false;
  }
 ...
 *                     
 */
function hasWhiteSpaces(version: unknown): boolean {
  if (typeof version != 'string') {
    return false;
  }
...
export function compareVersion(conditionsVersion: unknown, userProvidedVersion: unknown): number | null {
  // any version
  ...
  // console.log("split user parts: ", userVersionParts);
  // console.log("split conditions Parts parts: ", conditionsVersionParts);
